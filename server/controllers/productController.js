...
const{Product_name,product_descripton,Region,product_price,qty,Images,property,Category}=req.body;
...
module.exports.edit_products=async(req,res,next)=>{
   try{
    const{Product_name,product_descripton,Region,product_price,id,qty,Images, Category,property}=req.body;
...
module.exports.All_products=async(req,res,next)=>{
    try {
        ...
        const allproducts = await products.find({}).populate('Category').sort({'_id':-1});
        return res.json({status:true,allproducts});
    } catch (error) {
        ////console.error('Error fetching products:', error);
    }
...
module.exports.get_sorted_product=async(req,res,next)=>{
    try {
        ...
        const allproducts = await products.find({},null,{sort:{'_id':-1},limit:10});
    ...
    form.parse(req, async function (err, fields, files) {
        ...
        try {
            ...
            const uploadPromises = files.file.map(file => 
                cloudinary.uploader.upload(file.path, { folder: "nidz_ecommerce_website" })
            );
...
module.exports.Add_Categories = async (req, res, next) => {
    try{
        const {Catergoy_name,P}=req.body;
...
module.exports.get_category=async(req,res,next)=>{
        try {
            const{id}=req.body;
            const allproducts = await Categories.findOne({_id:id}).populate('parent');
           ////////console.log("updaing", allproducts)
            return res.json({status:true,allproducts});
        } catch (error) {
            ////console.error('Error fetching products:', error);
